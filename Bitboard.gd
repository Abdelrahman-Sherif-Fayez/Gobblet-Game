...
func get_external_gobbeling_moves(white_pieces, black_pieces, is_black_move):
    ...
    if is_black_move:
        var available_sizes = generate_path.get_available_external_sizes(black_pieces)
        var current_top_view_array = white_top_view
        for i in range(4):
            top_board |= white_top_view[i]
    else:
        var current_top_view_array = black_top_view
        var available_sizes = generate_path.get_available_external_sizes(white_pieces)
        for i in range(4):
