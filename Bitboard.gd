...
func get_external_gobbeling_moves(white_pieces, black_pieces, is_black_move):
	...
	var temp_moves = []
	var result = get_top_view_board_takes_input(white_pieces, black_pieces)
	...
	if is_black_move:
		var available_sizes = get_available_external_sizes(black_pieces)
		var current_top_view_array = white_top_view
		for i in range(4):
			top_board |= white_top_view[i]
	else:
		var current_top_view_array = black_top_view
		var available_sizes = get_available_external_sizes(white_pieces)
		for i in range(4):
	...
	return remove_duplicates(external_gobbeling_moves)
...
func generate_move_set(white_pieces: Array, black_pieces: Array, is_black_move: bool) -> Array:
	...
	var normal_external_moves = generate_path.get_external_moves(white_pieces, black_pieces, is_black_move)
...
func get_available_external_sizes(pieces):
	...
	var available_sizes = []
	var num_of_remaining_pieces = generate_path.get_remaining_pieces(pieces)
	for i in range(4):
		if i < 3:
